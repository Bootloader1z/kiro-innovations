...
• **Workspace Detection** (always) → **Reverse Engineering** (brownfield only) → **Requirements Analysis** (always, adaptive depth) → **Conditional Phases** (as needed) → **Workflow Planning** (always) → **Test Planning** (always, per-unit) → **Code Generation** (always, per-unit) → **Build and Test** (always)

## How It Works:
• **AI analyzes** your request, workspace, and complexity to determine which stages are needed
• **These stages always execute**: Workspace Detection, Requirements Analysis (adaptive depth), Workflow Planning, Test Planning (per-unit), Code Generation (per-unit), Build and Test
...
flowchart TD
    ...
    subgraph CONSTRUCTION["🟢 CONSTRUCTION PHASE"]
        ...
        TP["Test Planning<br/><b>ALWAYS</b>"]
    ...
    style TP fill:#4CAF50,stroke:#1B5E20,stroke-width:3px,color:#fff
...
- Test Planning: Formal test plan with scenarios, PBT properties, and pass/fail criteria (ALWAYS, per-unit)
