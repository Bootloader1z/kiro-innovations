...
### Test Planning (ALWAYS EXECUTE, per-unit)

**Always executes for each unit** — depth adapts based on unit complexity.

**Purpose**: Produce a formal Test Plan document that defines WHAT to test, HOW to test, and PASS/FAIL criteria before any code is written.

**Execution**:
1. **MANDATORY**: Log any user input during this stage in audit.md
2. Load all steps from `construction/test-planning.md`
3. Load context from prior stages (Functional Design, NFR Requirements, NFR Design, User Stories)
4. Generate test scenarios from acceptance criteria and business rules
5. Specify property-based test properties (if applicable)
6. Define test data requirements and pass/fail criteria
7. Create test plan at `aidlc-docs/construction/{unit-name}/test-plan/test-plan.md`
8. **MANDATORY**: Present standardized 2-option completion message as defined in test-planning.md - DO NOT use emergent behavior
9. **Wait for Explicit Approval**: User must choose between "Request Changes" or "Continue to Next Stage" - DO NOT PROCEED until user confirms
10. **MANDATORY**: Log user's response in audit.md with complete raw input
...
3. Load the approved Test Plan from `aidlc-docs/construction/{unit-name}/test-plan/test-plan.md` — use it to guide test implementation
...
│   │   │   ├── test-plan/           # Formal test plan (scenarios, PBT, pass/fail)
